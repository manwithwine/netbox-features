...
class VPNGroupFilterSet(NetBoxModelFilterSet):
    ...
    deputy = django_filters.ModelMultipleChoiceFilter(
        queryset=Contact.objects.all(),
        field_name='deputy',
        label="Deputy"
    )
    ...
    class Meta:
        model = VPNGroup
        fields = ['tenant', 'name', 'owner', 'deputy', 'acl', 'attribute', 'purpose', 'ttl', 'owneraudit', 'adminaudit']
